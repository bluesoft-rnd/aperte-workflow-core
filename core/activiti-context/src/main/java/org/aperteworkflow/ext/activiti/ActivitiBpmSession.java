...
public class ActivitiBpmSession extends AbstractProcessToolSession {
	...
	private static final Logger LOGGER = Logger
			.getLogger(ActivitiBpmSession.class.getName());
	...
	public ActivitiBpmSession(UserData user, Collection<String> roleNames) {
		super(user, roleNames, ProcessToolContext.Util
				.getThreadProcessToolContext().getRegistry());
		IdentityService is = getProcessEngine().getIdentityService();
		User bpmUser = is.createUserQuery().userId(user.getLogin())
				.singleResult();
		if (bpmUser == null) {
	...
	public void saveProcessInstance(ProcessInstance processInstance,
		...
		for (ProcessInstanceAttribute pia : processInstance
	...
	// @Override
	// public Collection<ProcessInstance> getQueueContents(final ProcessQueue
	// pq, final int offset, final int limit, ProcessToolContext ctx) {
	// Collection<ProcessQueue> configs = getUserQueuesFromConfig(ctx);
	// final List<String> names = keyFilter("name", configs);
	// if (!names.contains(pq.getName())) throw new
	// ProcessToolSecurityException("queue.no.rights", pq.getName());
	//
	...
	// final Map<String, ProcessInstance> instances =
	// ctx.getProcessInstanceDAO().getProcessInstanceByInternalIdMap(ids);
	...
	// @Override
	// public BpmTask assignTaskFromQueue(ProcessQueue q, BpmTask task,
	// ProcessToolContext ctx) {
	...
	@Override
	public void assignTaskToUser(ProcessToolContext ctx, String taskId,
			String userLogin) {
	...
	public BpmTask getTaskData(String taskId, ProcessToolContext ctx) {
		...
		List<BpmTask> tasks = findProcessInstancesForTasks(
				java.util.Collections.singletonList(task), ctx);
		return tasks.isEmpty() ? null : tasks.get(0);
	}

	@Override
	public BpmTask getTaskData(String taskExecutionId, String taskName,
			ProcessToolContext ctx) {
		List<Task> tasks = getProcessEngine().getTaskService()
				.createTaskQuery()
				// .notSuspended()
				.taskName(taskName).executionId(taskExecutionId)
				.taskAssignee(user.getLogin()).listPage(0, 1);
		if (tasks.isEmpty()) {
			logger.warning("Task " + taskExecutionId + " not found");
			return null;
		}
	...
	private List<BpmTask> findProcessInstancesForTasks(List<Task> tasks,
		...
		Map<String, List<Task>> tasksByProcessId = pl.net.bluesoft.util.lang.Collections
				.group(tasks, new Transformer<Task, String>() {
					@Override
					public String transform(Task task) {
						Execution exec = getProcessEngine().getRuntimeService()
		...
		final Map<String, ProcessInstance> instances = ctx
				.getProcessInstanceDAO().getProcessInstanceByInternalIdMap(
						tasksByProcessId.keySet());
		...
		for (final String processId : tasksByProcessId.keySet()) {
			...
			result.addAll(new Mapcar<Task, BpmTask>(processTasks) {
				...
				public BpmTask lambda(Task task) {
					...
					if (pi == null) {
						logger.warning("process " + processId + " not found");
						return null;
					}
	...
	private MutableBpmTask collectTask(Task task, ProcessInstance pi,
			ProcessToolContext ctx) {
	...
	public BpmTask refreshTaskData(BpmTask task, ProcessToolContext ctx) {
		MutableBpmTask bpmTask = task instanceof MutableBpmTask ? (MutableBpmTask) task
				: new MutableBpmTask(task);
		bpmTask.setProcessInstance(getProcessData(task.getProcessInstance()
				.getInternalId(), ctx));
		...
		List<Task> tasks = getProcessEngine().getTaskService()
				...
				.taskAssignee(user.getLogin()).listPage(0, 1);
		if (tasks.isEmpty()) {
			logger.warning("Task " + task.getExecutionId() + " not found");
	...
	public BpmTask getPastOrActualTask(ProcessInstanceLog log,
		...
		if (!taskNames.isEmpty()) // TODO what if there is > 1 task. Activiti
									// API does not support such operation (e.g.
									// in clause in SQL)?
		...
		for (HistoricTaskInstance task : tasks) {
			if (task.getEndTime() != null
					&& task.getEndTime().compareTo(minDate.getTime()) > 0) {
				pastTasks.add(task);
			}
		}
		if (!pastTasks.isEmpty()) {
			...
			List<BpmTask> actualTasks = findProcessTasks(pi, user.getLogin(),
					taskNames, ctx);
	...
	private MutableBpmTask collectHistoryActivity(HistoricTaskInstance task,
			ProcessInstance pi, UserData user) {
		MutableBpmTask t = new MutableBpmTask();
		t.setProcessInstance(pi);
	...
	private String findEndActivityName(ProcessInstance pi,
			ProcessToolContext ctx) {
		List<HistoricProcessInstance> history = getProcessEngine()
				.getHistoryService().createHistoricProcessInstanceQuery()
				.processInstanceId(pi.getInternalId()).list();
	...
	public List<BpmTask> findUserTasks(ProcessInstance processInstance,
		...
		List<Task> tasks = getProcessEngine().getTaskService()
				...
				.taskAssignee(user.getLogin()).listPage(0, 1000);
		return collectTasks(tasks, processInstance, ctx);
	}

	private List<BpmTask> collectTasks(List<Task> tasks,
			final ProcessInstance pi, final ProcessToolContext ctx) {
...
if(limit== null){
	...
	public List<BpmTask> findProcessTasks(ProcessInstance pi,
		...
		TaskQuery taskQuery = getProcessEngine().getTaskService()
				.createTaskQuery();
		taskQuery.processInstanceId(pi.getInternalId());
		if (user.getLogin() != null) {
			taskQuery.taskAssignee(user.getLogin());
		}
		List<Task> tasks = taskQuery.listPage(0, 1000);
	...
	public List<String> getOutgoingTransitionNames(String internalId,
		...
		org.activiti.engine.runtime.ProcessInstance pi = engine
				.getRuntimeService().createProcessInstanceQuery()
				.processInstanceId(internalId).singleResult();
		ExecutionImpl execution = (ExecutionImpl) pi;
		List<String> transitionNames = new ArrayList<String>();
		for (PvmTransition transition : execution.getActivity()
				.getOutgoingTransitions()) {
			System.out.println(transition.getDestination().getId()); // TODO
																		// zmienic
																		// getOutgoingTransitionName
																		// w ten
																		// sam
																		// sposob
																		// co w
	...
	@Override
	public List<String> getOutgoingTransitionDestinationNames(
			String internalId, ProcessToolContext ctx) {
		ProcessEngine engine = getProcessEngine();
		org.activiti.engine.runtime.ProcessInstance pi = engine
				.getRuntimeService().createProcessInstanceQuery()
				.processInstanceId(internalId).singleResult();
	...
	protected ProcessEngine getProcessEngine() {
		ProcessToolContext ctx = ProcessToolContext.Util
				.getThreadProcessToolContext();
		if (ctx instanceof ActivitiContextImpl) {
			ProcessEngine engine = ((ActivitiContextImpl) ctx)
					.getProcessEngine();
			if (user != null && user.getLogin() != null)
				engine.getIdentityService().setAuthenticatedUserId(
						user.getLogin());
			return engine;
		} else {
			throw new IllegalArgumentException(ctx + " not an instance of "
					+ ActivitiContextImpl.class.getName());
		}
	}

	public Collection<ProcessInstance> getUserProcesses(int offset, int limit,
			ProcessToolContext ctx) {
		List<Task> taskList = getProcessEngine().getTaskService()
				.createTaskQuery().taskAssignee(user.getLogin())
				.orderByExecutionId().desc().listPage(offset, limit);

		List<String> ids = keyFilter("processInstanceId", taskList);
		final Map<String, ProcessInstance> instances = ctx
				.getProcessInstanceDAO().getProcessInstanceByInternalIdMap(ids);
		return new Mapcar<Task, ProcessInstance>(taskList) {
			...
			public ProcessInstance lambda(Task task) {
				ProcessInstance pi = instances.get(task.getProcessInstanceId());
				if (pi == null) {
					logger.warning("process " + task.getProcessInstanceId()
							+ " not found");
					return null;
				}
				pi.setState(task.getName());
	...
	public Collection<ProcessQueue> getUserAvailableQueues(
		...
		for (ProcessQueue q : configs) {
			q.setProcessCount(ts.createTaskQuery()
					.taskCandidateGroup(q.getName()).taskUnnassigned().count());
		}
		return configs;
	}
	...
	public BpmTask assignTaskFromQueue(final ProcessQueue pq, BpmTask pi,
		...
		if (!names.contains(pq.getName()))
			throw new ProcessToolSecurityException("queue.no.rights",
					pq.getName());
		TaskService ts = getProcessEngine().getTaskService();

		Task task;
		if (taskId == null) {
			task = ts.createTaskQuery().taskCandidateGroup(pq.getName())
					.taskUnnassigned().orderByExecutionId().desc()
					.singleResult();
		} else {
			task = ts.createTaskQuery().taskId(taskId)
					.taskCandidateGroup(pq.getName()).taskUnnassigned()
					.singleResult();
		}
		if (task == null) {
			return null;
		...
		ProcessInstance pi2 = ctx.getProcessInstanceDAO()
				.getProcessInstanceByInternalId(task.getProcessInstanceId());
		if (pi2 == null) {
			if (pi == null)
		...
		if (!user.getLogin().equals(
				ts.createTaskQuery().taskId(task.getId()).singleResult()
						.getAssignee())) {
			if (pi == null)
				return assignTaskFromQueue(pq, ctx);
			else
		...
		log.setState(ctx.getProcessDefinitionDAO()
		...
		eventBusManager.publish(new BpmEvent(BpmEvent.Type.ASSIGN_TASK, pi2,
				user));

		ctx.getEventBusManager().publish(
				new BpmEvent(BpmEvent.Type.ASSIGN_TASK, pi2, user));
	...
	private void fillProcessAssignmentData(final ProcessInstance pi,
		...
		for (Task t : findProcessTasks(pi, ctx, false)) {
			if (t.getAssignee() != null) {
				...
				for (IdentityLink participation : taskService
	...
	// @Override()
	// public Collection<BpmTask> getTaskList(ProcessInstance pi, final
	// ProcessToolContext ctx) {
	...
	// @Override()
	// public Collection<BpmTask> getTaskList(ProcessInstance pi, final
	// ProcessToolContext ctx, final boolean mustHaveAssignee) {
	...
	// private List<BpmTask> findProcessTasks(final ProcessInstance
	...
	@Override
	public List<BpmTask> findProcessTasks(ProcessInstance pi,
			final String userLogin, ProcessToolContext ctx) {
		return findProcessTasks(pi, userLogin, null, ctx);
	}

	@Override
	public List<BpmTask> findProcessTasks(ProcessInstance pi, String userLogin,
			Set<String> taskNames, ProcessToolContext ctx) {

		TaskQuery q = getProcessEngine().getTaskService().createTaskQuery()
				.processInstanceId(pi.getInternalId());
		if (userLogin != null)
			q = q.taskAssignee(userLogin);
		if (taskNames != null && !taskNames.isEmpty()) // TODO what if more than
														// 1 task name is
														// supplied
			q = q.taskName(taskNames.iterator().next());
	...
	@Override
	public ResultsPageWrapper<BpmTask> findProcessTasks(
			ProcessInstanceFilter filter, Integer offset, Integer limit,
			ProcessToolContext ctx) {
		/*
		 * criteria: - assignee logins that must be assigned [LS] AND - assignee
		 * logins that must not be assigned [LS] AND - task lifecycle states AND
		 * [ignore] - queue names AND [LS] - task names [LS]
		 */
		...
		List<Task> tasks = commandExecutorTxRequired
				.execute(new Command<List<Task>>() {
					@Override
					public List<Task> execute(CommandContext commandContext) {
						return commandContext.getDbSqlSession().selectList(
								"selectTaskByQueryCriteria_Enhanced", q);
					}
				});
		return new ResultsPageWrapper<BpmTask>(findProcessInstancesForTasks(
				tasks, ctx));// TODO count total

		// configuration.

		// final Collection ownerNames =
		// pl.net.bluesoft.util.lang.Collections.collect(filter.getOwners(), new
		// Transformer<UserData, String>() {
		// @Override
		...
		// });
		//
		// final Collection notOwnerNames =
		// pl.net.bluesoft.util.lang.Collections.collect(filter.getNotOwners(),
		// new Transformer<UserData, String>() {
		// @Override
		// public String transform(UserData obj) {
		// return obj.getLogin();
		// }
		...
		// }
		//
		// Command<List<HistoricTaskInstance>> cmd = new
		// Command<List<HistoricTaskInstance>>() {
		// @Override
		// public List<HistoricTaskInstance> execute(CommandContext
		// commandContext) {
		...
		// /*hql.append("left join fetch act.historyProcessInstance ")*/
		...
		// processEngine.get
		//
		// Map<String, List<HistoryTaskInstanceImpl>> tasksByProcessId =
		// pl.net.bluesoft.util.lang.Collections.group(tasks, new
		// Transformer<HistoryTaskInstanceImpl, String>() {
		// @Override
	...
	private ActivitiContextFactoryImpl.CustomStandaloneProcessEngineConfiguration getProcessEngineConfiguration() {
		ProcessToolContext ctx = ProcessToolContext.Util
				.getThreadProcessToolContext();
		if (ctx instanceof ActivitiContextImpl) {
			return ((ActivitiContextImpl) ctx)
					.getCustomStandaloneProcessEngineConfiguration();
		} else {
			throw new IllegalArgumentException(ctx + " not an instance of "
					+ ActivitiContextImpl.class.getName());
		}
	...
	@Override
	public ResultsPageWrapper<BpmTask> findRecentTasks(Calendar minDate,
			Integer offset, Integer limit, ProcessToolContext ctx) {
		List<BpmTask> recentTasks = new ArrayList<BpmTask>();
		UserData user = getUser(ctx);
		ResultsPageWrapper<ProcessInstance> recentInstances = ctx
				.getProcessInstanceDAO().getRecentProcesses(user, minDate,
						offset, limit);
		List<ProcessInstance> instances = recentInstances.getResults();
		for (ProcessInstance pi : instances) {
			List<BpmTask> tasks = findProcessTasks(pi, user.getLogin(), ctx);
			if (tasks.isEmpty()) {
				BpmTask task = getMostRecentProcessHistoryTask(pi, user,
						minDate, ctx);
				if (task != null) {
	...
	private BpmTask getMostRecentProcessHistoryTask(final ProcessInstance pi,
			final UserData user, final Calendar minDate, ProcessToolContext ctx) {
	...
	public Integer getRecentTasksCount(Calendar minDate, ProcessToolContext ctx) {
		int count = 0;
		UserData user = getUser(ctx);
		Collection<ProcessInstance> instances = ctx.getProcessInstanceDAO()
				.getUserProcessesAfterDate(user, minDate);
		for (ProcessInstance pi : instances) {
			List<BpmTask> tasks = findProcessTasks(pi, user.getLogin(), ctx);
			if (tasks.isEmpty()
					&& getMostRecentProcessHistoryTask(pi, user, minDate, ctx) != null) {
				count++;
			} else {
				count += tasks.size();
			}
	...
	public Collection<BpmTask> getAllTasks(ProcessToolContext ctx) {
		return findProcessInstancesForTasks(getProcessEngine().getTaskService()
				.createTaskQuery().orderByTaskId().desc().listPage(0, 1000),
				ctx);
	}

	// @Override
	// public BpmTask performAction(ProcessStateAction action, BpmTask bpmTask,
	// ProcessToolContext ctx) {
	...
	// private Task findSubProcessTask(final ProcessInstance processInstance,
	// ProcessToolContext ctx) {
	// ProcessInstanceQuery subprocessQuery =
	// getProcessEngine().getRuntimeService().createProcessInstanceQuery().superProcessInstanceId(processInstance.getInternalId());
	// org.activiti.engine.runtime.ProcessInstance subprocess =
	// subprocessQuery.singleResult();
	// return subprocess == null ? null :
	...
	public BpmTask performAction(ProcessStateAction action,
			ProcessToolContext ctx, BpmTask bpmTask) {
		return performAction(
				action,
				bpmTask.getProcessInstance(),
				ctx,
				getProcessEngine().getTaskService().createTaskQuery()
						.taskId(bpmTask.getInternalTaskId()).singleResult());
	}

	private BpmTask performAction(ProcessStateAction action,
			ProcessInstance processInstance, ProcessToolContext ctx, Task task) {
		...
		if (s == null && processInstance.getRunning()
				&& !isProcessRunning(processInstance.getInternalId(), ctx)) {
			processInstance.setRunning(false);
		}
		ctx.getProcessInstanceDAO().saveProcessInstance(processInstance);
		publishEvents(processInstance,
				processInstance.getRunning() ? BpmEvent.Type.SIGNAL_PROCESS
						: BpmEvent.Type.END_PROCESS);
		...
		return collectTask(findProcessTask(processInstance, ctx),
				processInstance, ctx);
	}
	...
	public void updateSubprocess(final ProcessInstance parentPi,
		...
		ProcessInstanceQuery subprocessQuery = service
				.createProcessInstanceQuery().superProcessInstanceId(
						parentPi.getInternalId());
		org.activiti.engine.runtime.ProcessInstance subprocess = subprocessQuery
				.singleResult();
		if (subprocess != null) {
			String processDefinitionId = subprocess.getProcessDefinitionId()
					.replaceFirst(":\\d+:\\d+$", "");
			ProcessDefinitionConfig config = ctx.getProcessDefinitionDAO()
					.getActiveConfigurationByKey(processDefinitionId);
			pl.net.bluesoft.rnd.processtool.model.ProcessInstance subPi = createProcessInstance(
					config, null, ctx, null, null, "parent_process",
					subprocess.getId());
	...
	private void publishEvents(ProcessInstance processInstance,
			BpmEvent.Type signalProcess) {
		eventBusManager.publish(new BpmEvent(signalProcess, processInstance,
				user));
		ProcessToolContext.Util.getThreadProcessToolContext()
				.getEventBusManager()
				.publish(new BpmEvent(signalProcess, processInstance, user));
	}

	private void addActionLogEntry(ProcessStateAction action,
			ProcessInstance processInstance, ProcessToolContext ctx) {
		// TODO
		// ProcessStateConfiguration state =
		// ctx.getProcessDefinitionDAO().getProcessStateConfiguration(processInstance);
		...
		log.setAdditionalInfo(nvl(action.getLabel(), action.getDescription(),
				action.getBpmName()));
		log.setUser(ctx.getUserDataDAO().findOrCreateUser(user));
		log.setUserSubstitute(substitutingUser != null ? ctx.getUserDataDAO()
				.findOrCreateUser(substitutingUser) : null);
	...
	public boolean isProcessRunning(String internalId, ProcessToolContext ctx) {
		...
		RuntimeService service = getProcessEngine().getRuntimeService();
		org.activiti.engine.runtime.ProcessInstance processInstance = service
				.createProcessInstanceQuery().processInstanceId(internalId)
				.singleResult();
	...
	protected ProcessInstance startProcessInstance(
			ProcessDefinitionConfig config, String externalKey,
		...
		for (ProcessInstanceAttribute pia : pi.getProcessAttributes()) {
			if (pia instanceof BpmVariable) {
				BpmVariable bpmVar = (BpmVariable) pia;
				if (hasText(bpmVar.getBpmVariableName())) {
					vars.put(bpmVar.getBpmVariableName(),
							bpmVar.getBpmVariableValue());
				}
			}
		}
	...
	public ProcessToolBpmSession createSession(UserData user,
			Collection<String> roleNames, ProcessToolContext ctx) {
	...
	public void adminCancelProcessInstance(ProcessInstance pi) {
		logger.severe("User: " + user.getLogin()
				+ " attempting to cancel process: " + pi.getInternalId());
		ProcessToolContext ctx = ProcessToolContext.Util
				.getThreadProcessToolContext();
		pi = getProcessData(pi.getInternalId(), ctx);
		ProcessEngine processEngine = getProcessEngine();
		processEngine.getRuntimeService().deleteProcessInstance(
				pi.getInternalId(), "admin-cancelled");
		fillProcessAssignmentData(pi, ctx);
		pi.setRunning(false);
		pi.setState(null);
		ctx.getProcessInstanceDAO().saveProcessInstance(pi);
		logger.severe("User: " + user.getLogin() + " has cancelled process: "
				+ pi.getInternalId());

	}

	@Override
	public void adminReassignProcessTask(ProcessInstance pi, BpmTask bpmTask,
			String userLogin) {
		logger.severe("User: " + user.getLogin()
				+ " attempting to reassign task " + bpmTask.getInternalTaskId()
				+ " for process: " + pi.getInternalId() + " to user: "
				+ userLogin);
		...
		Task task = ts.createTaskQuery().taskId(bpmTask.getInternalTaskId())
				.singleResult();
		if (nvl(userLogin, "").equals(nvl(task.getAssignee(), ""))) {
			logger.severe("User: " + user.getLogin()
					+ " has not reassigned task " + bpmTask.getInternalTaskId()
					+ " for process: " + pi.getInternalId()
					+ " as the user is the same: " + userLogin);
			return;
		}
		// this call should also take care of swimlanes
		ts.setAssignee(bpmTask.getInternalTaskId(), userLogin);
		fillProcessAssignmentData(pi, ctx);
		logger.info("Process.running:" + pi.getRunning());
		ctx.getProcessInstanceDAO().saveProcessInstance(pi);
		logger.severe("User: " + user.getLogin() + " has reassigned task "
				+ bpmTask.getInternalTaskId() + " for process: "
				+ pi.getInternalId() + " to user: " + userLogin);

	}

	@Override
	public void adminCompleteTask(ProcessInstance pi, BpmTask bpmTask,
			ProcessStateAction action) {
		logger.severe("User: " + user.getLogin()
				+ " attempting to complete task " + bpmTask.getInternalTaskId()
				+ " for process: " + pi.getInternalId() + " to outcome: "
				+ action);
		performAction(action,
				ProcessToolContext.Util.getThreadProcessToolContext(), bpmTask);
		logger.severe("User: " + user.getLogin() + " has completed task "
				+ bpmTask.getInternalTaskId() + " for process: "
				+ pi.getInternalId() + " to outcome: " + action);
	...
	public List<String> getAvailableLogins(final String filter) {
		...
		List<User> users = is.createUserQuery()
				.userEmailLike("%" + filter + "%").orderByUserId().asc()
				.listPage(0, 100);
	...
	public List<GraphElement> getProcessHistory(final ProcessInstance pi) {
		...
		HistoricActivityInstanceQuery activityInstanceQuery = service
		...
		Collections.sort(list, new Comparator<HistoricActivityInstance>() {
			...
			public int compare(HistoricActivityInstance o1,
				...
				return nvl(new Date(), o1.getStartTime()).compareTo(
						nvl(new Date(), o2.getStartTime()));
			}
		});
		...
		for (HistoricActivityInstance activity : list) {
			...
			if (prev != null) {
				TransitionArc ta = (TransitionArc) processGraphElements
						.get("__AWF__" + prev.getActivityName() + "_"
								+ activityName);
				if (ta == null) { // look for default!
					ta = (TransitionArc) processGraphElements
							.get("__AWF__default_transition_"
									+ prev.getActivityName());
				}
				if (ta != null) {
			...
			// GraphElement startNode =
			// processGraphElements.get("__AWF__start_node");
			// if (startNode != null) {
			// res.add(startNode);
			// }
			// GraphElement firstTransition =
			// processGraphElements.get("__AWF__start_transition_to_" +
			// activityName);
			...
			// sn.setLabel(activityName + ": " + activity.getDurationInMillis()
			// + "ms");
			res.add(sn);
			// look for transition
		}

		HistoricProcessInstanceQuery historyProcessInstanceQuery = getProcessEngine()
				.getHistoryService().createHistoricProcessInstanceQuery()
				.processInstanceId(pi.getInternalId());
		HistoricProcessInstance historyProcessInstance = historyProcessInstanceQuery
				.singleResult();
		if (historyProcessInstance != null
				&& historyProcessInstance.getEndActivityId() != null) {
			StateNode sn = (StateNode) processGraphElements
					.get(historyProcessInstance.getEndActivityId());
			if (sn != null) {
				if (prev != null) {
					TransitionArc ta = (TransitionArc) processGraphElements
							.get("__AWF__" + prev.getActivityName() + "_"
	...
	private HashMap<String, GraphElement> parseProcessDefinition(
			ProcessInstance pi) {
		...
		try {
			...
			Document dom = db
					.parse(new ByteArrayInputStream(processDefinition));
			Element documentElement = dom.getDocumentElement();

			NodeList diagrams = documentElement.getElementsByTagNameNS(
					BPMNDI_NAMESPACE, "BPMNDiagram");
			if (diagrams.getLength() == 0) {
				logger.severe("No diagram data for process definition for instance "
						+ pi.getInternalId());
				return res;
			}
			Element diagramElement = (Element) diagrams.item(0);
			NodeList planes = diagramElement.getElementsByTagNameNS(
					BPMNDI_NAMESPACE, "BPMNPlane");
			if (planes.getLength() == 0) {
				logger.severe("No plane data for process definition for instance "
						+ pi.getInternalId());
				return res;
			}
			...
			NodeList processElements = documentElement
					.getElementsByTagName("process");
			if (processElements.getLength() == 0) {
				logger.severe("No process data for process definition for instance "
						+ pi.getInternalId());
			}
			Element processElement = (Element) processElements.item(0);

			String[] nodeTypes = new String[] { "startEvent", "userTask",
					"exclusiveGateway", "serviceTask", "endEvent" };
			...
			/*
			 * <sequenceFlow id="sid-1D101C48-E36A-4A42-8E95-5C83E9A4CE62"
			 * name="Continue"
			 * sourceRef="sid-7BAAA2C8-B1B8-44C7-8C4F-D3968FBAD1B9"
			 * targetRef="sid-9EB37DC6-A17D-4F6B-AD72-64481593034E">
			 * <conditionExpression
			 * id="sid-754c93dd-7b08-4c0b-b858-848069d98e4d"
			 * xsi:type="tFormalExpression"
			 * >${ACTION=='Continue'}</conditionExpression> </sequenceFlow>
			 */
			...
			for (String nodeType : nodeTypes) {
				...
				for (int i = 0; i < nodes.getLength(); i++) {
					Element node = (Element) nodes.item(i);
					try {
						TransitionArc arc = arcById
								.get(node.getAttribute("id"));
						if (arc == null) {
							continue;
						}
						String name = node.getAttribute("name");
						arc.setName(name);
						res.put(name, arc);
						StateNode startNode = nodeById.get(node
								.getAttribute("sourceRef"));
						StateNode endNode = nodeById.get(node
								.getAttribute("targetRef"));
						if (startNode == null || endNode == null) {
							continue;
						}
						// calculate line start node which is a center of the
						// start node
						int startX = startNode.getX() + startNode.getWidth()
								/ 2;
						int startY = startNode.getY() + startNode.getHeight()
								/ 2;
						...
						arc.getPath().add(0,
								new TransitionArcPoint(startX, startY));
						...
						if (startX - endX == 0) { // whoa - vertical line -
						...
						endX = arc.getPath().get(arc.getPath().size() - 1)
								.getX();
						endY = arc.getPath().get(arc.getPath().size() - 1)
								.getY();
						startX = arc.getPath().get(arc.getPath().size() - 2)
								.getX();
						startY = arc.getPath().get(arc.getPath().size() - 2)
								.getY();
						if (startX - endX == 0) { // whoa - vertical line -
													// simple case, but requires
													// special approach
						...
						res.put("__AWF__" + startNode.getLabel() + "_"
								+ endNode.getLabel(), arc);
						res.put("__AWF__default_transition_"
								+ startNode.getLabel(), arc);
						LOGGER.fine("Found node" + name);
					} catch (Exception e) {
						LOGGER.log(Level.SEVERE, e.getMessage(), e);
					}
	...
	private Map<String, StateNode> getElementCoordinatesMap(Element planeElement) {
		...
		NodeList nodes = planeElement.getElementsByTagNameNS(BPMNDI_NAMESPACE,
				"BPMNShape");
		for (int i = 0; i < nodes.getLength(); i++) {
			Element node = (Element) nodes.item(i);
			try {
				/*
				 * <bpmndi:BPMNShape
				 * bpmnElement="sid-0085EC77-5E5A-41F2-AB86-1323CDAA63B9"
				 * id="sid-0085EC77-5E5A-41F2-AB86-1323CDAA63B9_gui">
				 * <omgdc:Bounds height="30.0" width="30.0" x="21.0" y="89.0"/>
				 * </bpmndi:BPMNShape>
				 */
				StateNode sn = new StateNode();
				sn.setId(node.getAttribute("bpmnElement"));
				NodeList boundsList = node.getElementsByTagNameNS(OMG_DC_URI,
						"Bounds");
				...
				LOGGER.fine("Found node" + sn.getId() + ": " + x + "," + y
						+ "," + w + "," + h);
			} catch (Exception e) {
				LOGGER.log(Level.SEVERE, e.getMessage(), e);
			}
	...
	private Map<String, TransitionArc> getArcCoordinatesMap(Element planeElement) {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			try {
				/*
				 * <bpmndi:BPMNEdge
				 * bpmnElement="sid-581B2A50-275F-4790-BA45-FC2728E3E8F7"
				 * id="sid-581B2A50-275F-4790-BA45-FC2728E3E8F7_gui">
				 * <omgdi:waypoint x="552.0" y="198.0"/> <omgdi:waypoint
				 * x="552.3181702913334" y="60.0"/> <omgdi:waypoint x="274.0"
				 * y="60.0"/> </bpmndi:BPMNEdge>
				 */
				TransitionArc ta = new TransitionArc();
				ta.setId(node.getAttribute("bpmnElement"));
				NodeList waypointList = node.getElementsByTagNameNS(
						"http://www.omg.org/spec/DD/20100524/DI", "waypoint");
				if (waypointList.getLength() == 0) {
					continue;
				}
				for (int j = 1; j < waypointList.getLength() - 1; j++) {// skip
																		// first
																		// and
																		// last
																		// docker
					...
					ta.addPoint(
							new Double(waypointEl.getAttribute("x")).intValue(),
	...
	public byte[] getProcessDefinition(ProcessInstance pi) {
		String resourceName = pi.getDefinition().getProcessName()
				+ ".bpmn20.xml";
		return fetchProcessResource(pi, resourceName);
	}

	private byte[] fetchLatestProcessResource(String definitionKey,
			String resourceName) {
		RepositoryService service = getProcessEngine().getRepositoryService();
		List<ProcessDefinition> latestList = service
				.createProcessDefinitionQuery()
				.processDefinitionKey(definitionKey).orderByDeploymentId()
				.desc().listPage(0, 1);
	...
	private byte[] fetchProcessResource(ProcessInstance pi, String resourceName) {
		...
		org.activiti.engine.runtime.ProcessInstance processInstanceById = RuntimeService
				.createProcessInstanceQuery()
				.processInstanceId(pi.getInternalId()).singleResult();
		String processDefinitionId;
		if (processInstanceById == null) { // look in history service
			HistoricProcessInstanceQuery historyProcessInstanceQuery = processEngine
					.getHistoryService().createHistoricProcessInstanceQuery()
					.processInstanceId(pi.getInternalId());
			HistoricProcessInstance historyProcessInstance = historyProcessInstanceQuery
					.singleResult();
			processDefinitionId = historyProcessInstance
		...
		List<ProcessDefinition> latestList = service
				.createProcessDefinitionQuery()
				.processDefinitionId(processDefinitionId).orderByDeploymentId()
				.desc().listPage(0, 1);
		if (!latestList.isEmpty()) {
			String oldDeploymentId = latestList.get(0).getDeploymentId();
			return getDeploymentResource(resourceName, oldDeploymentId);
	...
	private byte[] getDeploymentResource(String resourceName,
			String oldDeploymentId) {
		RepositoryService service = getProcessEngine().getRepositoryService();
		;
		try {
			InputStream oldStream = service.getResourceAsStream(
					oldDeploymentId, resourceName);
			try {
				if (oldStream != null) {
	...
	public String deployProcessDefinition(String processName,
			InputStream definitionStream, InputStream processMapImageStream) {
		RepositoryService service = getProcessEngine().getRepositoryService();
		DeploymentBuilder deployment = service.createDeployment();
		// deployment.name(processName);
		deployment
				.addInputStream(processName + ".bpmn20.xml", definitionStream);
		if (processMapImageStream != null)
			deployment.addInputStream(processName + ".png",
					processMapImageStream);
