...
//		taskList.setMargin(true);
		...
		if (bpmEventListener == null) {
			activityMainPane.getBpmSession().getEventBusManager().subscribe(BpmEvent.class,
			                                                                bpmEventListener = new EventListener<BpmEvent>() {
				@Override
	...
	public void refreshData() {
		...
		if(!substitutedUsers.isEmpty()){
			Map<UserData, ProcessToolBpmSession> substitutedUserToSession = from(substitutedUsers).mapTo(
	...
	private void buildMainTasksViews(ProcessToolContext ctx, final ProcessToolBpmSession bpmSession, UserData user) {
		final ProcessInstanceFilter assignedTasksFromOthers = getProcessInstanceFilter(user, null, user, getMessage("activity.assigned.tasks"), TaskState.OPEN);
		taskList.addComponent(createUserTasksButton(bpmSession, ctx, assignedTasksFromOthers, true));
		final ProcessInstanceFilter assignedTasksByMyself = getProcessInstanceFilter(user, user, user, getMessage("activity.created.assigned.tasks"), TaskState.OPEN);
		taskList.addComponent(createUserTasksButton(bpmSession, ctx, assignedTasksByMyself, true));
		final ProcessInstanceFilter myTasksBeingDoneByOthers = getProcessInstanceFilter(user, user, null, getMessage("activity.created.tasks"), TaskState.OPEN);
	...
	private int buildSubstitutedTasks(ProcessToolContext ctx, final ProcessToolBpmSession bpmSessionForSubstituted, UserData user,
		...
		final ProcessInstanceFilter substAssignedTasksFromOthers = getProcessInstanceFilter(user, null, user, getMessage("activity.subst.assigned.tasks"), TaskState.OPEN);		
		final ProcessInstanceFilter substAssignedTasksByMyself = getProcessInstanceFilter(user, user, user, getMessage("activity.subst.created.assigned.tasks"), TaskState.OPEN);		
		final ProcessInstanceFilter substMyTasksBeingDoneByOthers = getProcessInstanceFilter(user, user, null, getMessage("activity.subst.created.tasks"), TaskState.OPEN);			
		final ProcessInstanceFilter substMyTasksClosed = getProcessInstanceFilter(user, user, null, getMessage("activity.subst.created.closed.tasks"), TaskState.CLOSED);
		
		int total = 0;
		
		for(ProcessInstanceFilter filter : new ProcessInstanceFilter[]{substAssignedTasksFromOthers, substAssignedTasksByMyself, substMyTasksBeingDoneByOthers, substMyTasksClosed}){
	...
	private void buildSubstitutedTasksViews(ProcessToolContext ctx, List<UserData> substitutedUsers, Map<UserData, ProcessToolBpmSession> substitutedUserToSession, List<ProcessQueue> userAvailableQueues) {
		...
		for (UserData substitutedUser : substitutedUsers) {
			...
			final ProcessToolBpmSession bpmSessionForSubstituted = substitutedUserToSession.get(substitutedUser);
			...
			final ProcessInstanceFilter substAssignedTasks = getProcessInstanceFilter(substitutedUser, null, liferayUser, getMessage("activity.other.users.tasks", liferayUser.getRealName()), TaskState.OPEN);
			...
			container.getItem(substAssignedTasks).getItemProperty("description").setValue(getMessage("activity.substitutions.description", liferayUser.getRealName(), total, totalQueues));
	...
	private int buildSubstitutedQueues(ProcessToolContext ctx, final List<ProcessQueue> userAvailableQueues,
			ProcessToolBpmSession bpmSessionForSubstituted, UserData user, HierarchicalContainer container, ProcessInstanceFilter parent) {
		...
		Collection<String> substitutedUserQueueNames = from(map.keySet())
				.except(from(userAvailableQueues).select(new F<ProcessQueue, String>(){
					@Override
	...
	private Button createUserTasksButton(final ProcessToolBpmSession bpmSession, final ProcessToolContext ctx, final ProcessInstanceFilter processInstanceFilter, final boolean showCounter) {
...
//		pif.setCreators(CollectionUtils.collect(pif.getCreators(), transformer)
...
//    }
    ...
    private Button createRecentTasksButton(ProcessToolContext ctx) {
		...
		Button b = new Button(getMessage("activity.recent.tasks"));// + " (" + recentProcessesSize + ")");
		b.setStyleName(BaseTheme.BUTTON_LINK);
	...
	private static List<UserData> getSubstitutedUsers(UserData user, Date date) {
		List<UserData> substitutedUsers = ProcessToolContext.Util.getThreadProcessToolContext()
				.getUserSubstitutionDAO()
				.getSubstitutedUsers(user, DateUtil.truncHours(date));
		return from(substitutedUsers)
				.select(new F<UserData, UserData>() {
					@Override
