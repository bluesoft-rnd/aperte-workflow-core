...
public class ProcessMultiViewDataPane extends VerticalLayout implements WidgetContextSupport 
{
	...
	private Logger logger = Logger.getLogger(ProcessMultiViewDataPane.class.getName());
	...
	public ProcessMultiViewDataPane(Application application, ProcessToolBpmSession bpmSession, I18NSource i18NSource, BpmTask bpmTask,
	...
	private void initLayout(boolean autoHide) {
		...
		if (isOwner) 
		{
			AbstractLayout upperButtonsLayout = getButtonsPanel(stateConfiguration);
			upperButtonsLayout.addStyleName(TOP_BUTTONS_PANEL_STYLE);
			addComponentAsFirst(upperButtonsLayout);

			AbstractLayout bottomButtonsLayout = getButtonsPanel(stateConfiguration);
	...
	public Map<ProcessToolDataWidget, Collection<String>> getWidgetsErrors(BpmTask bpmTask, boolean skipRequired) {
		Map<ProcessToolDataWidget, Collection<String>> errorMap = new HashMap<ProcessToolDataWidget, Collection<String>>();
	...
	private class MyProcessToolActionCallback implements ProcessToolActionCallback, Serializable {
		private void actionCompleted(GuiAction guiAction, ProcessStateAction action) {
			ProcessMultiViewDataPane.this.guiAction = guiAction;
			refreshTask();
		...
		public WidgetContextSupport getWidgetContextSupport() {
			return ProcessMultiViewDataPane.this;
		}
	}
}
